import crypto from 'crypto'

const ALGORITHM = 'aes-256-gcm'
const IV_LENGTH = 16
const SALT_LENGTH = 64
const TAG_LENGTH = 16
const TAG_POSITION = SALT_LENGTH + IV_LENGTH
const ENCRYPTED_POSITION = TAG_POSITION + TAG_LENGTH

const ENCRYPTION_KEY = 'bb4459589152ede4cf631961f6fcc6f670538ff6e9966cb23ee852c230822cbb'
const ENCRYPTED_TOKEN = ''
const ORG_ID = '21579434'

function decryptToken(encryptedText) {
  const data = Buffer.from(encryptedText, 'base64')
  const salt = data.subarray(0, SALT_LENGTH)
  const iv = data.subarray(SALT_LENGTH, TAG_POSITION)
  const tag = data.subarray(TAG_POSITION, ENCRYPTED_POSITION)
  const encrypted = data.subarray(ENCRYPTED_POSITION)
  const key = crypto.pbkdf2Sync(ENCRYPTION_KEY, salt, 100000, 32, 'sha512')
  const decipher = crypto.createDecipheriv(ALGORITHM, key, iv)
  decipher.setAuthTag(tag)
  return decipher.update(encrypted) + decipher.final('utf8')
}

async function test() {
  const accessToken = decryptToken(ENCRYPTED_TOKEN)
  console.log('Token decrypted, first 30 chars:', accessToken.substring(0, 30) + '...')

  // Test 1: Follower stats (should work)
  console.log('\n=== Test 1: Follower Stats (should work) ===')
  const followerUrl = `https://api.linkedin.com/rest/organizationalEntityFollowerStatistics?q=organizationalEntity&organizationalEntity=urn:li:organization:${ORG_ID}`
  const r1 = await fetch(followerUrl, {
    headers: {
      'Authorization': `Bearer ${accessToken}`,
      'X-Restli-Protocol-Version': '2.0.0',
      'LinkedIn-Version': '202401',
    }
  })
  console.log('Status:', r1.status)
  const t1 = await r1.text()
  console.log('Response:', t1.substring(0, 300))

  // Test 2: Page Stats with REST API (403?)
  console.log('\n=== Test 2: Page Stats - REST API ===')
  const pageStatsUrl = `https://api.linkedin.com/rest/organizationPageStatistics?q=organization&organization=urn:li:organization:${ORG_ID}`
  const r2 = await fetch(pageStatsUrl, {
    headers: {
      'Authorization': `Bearer ${accessToken}`,
      'X-Restli-Protocol-Version': '2.0.0',
      'LinkedIn-Version': '202401',
    }
  })
  console.log('Status:', r2.status)
  const t2 = await r2.text()
  console.log('Response:', t2.substring(0, 500))

  // Test 3: Page Stats with v2 API
  console.log('\n=== Test 3: Page Stats - v2 API ===')
  const pageStatsV2Url = `https://api.linkedin.com/v2/organizationPageStatistics?q=organization&organization=urn:li:organization:${ORG_ID}`
  const r3 = await fetch(pageStatsV2Url, {
    headers: {
      'Authorization': `Bearer ${accessToken}`,
      'X-Restli-Protocol-Version': '2.0.0',
    }
  })
  console.log('Status:', r3.status)
  const t3 = await r3.text()
  console.log('Response:', t3.substring(0, 500))

  // Test 4: Page Stats without version header
  console.log('\n=== Test 4: Page Stats - No version header ===')
  const r4 = await fetch(pageStatsUrl, {
    headers: {
      'Authorization': `Bearer ${accessToken}`,
      'X-Restli-Protocol-Version': '2.0.0',
    }
  })
  console.log('Status:', r4.status)
  const t4 = await r4.text()
  console.log('Response:', t4.substring(0, 500))
}

test().catch(console.error)
